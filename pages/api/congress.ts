...
export default apiHandler(["GET"], async (req, res: NextApiResponse<CongressResponse | { error: string }>) => {
  ...
  try {
    ...
    // Allow callers (e.g. watchlist / proposed positions) to request extra
    // tickers beyond what's held. Union held + requested, de-duped.
    const requested =
      typeof req.query.tickers === "string"
        ? req.query.tickers.split(",").map(t => t.trim().toUpperCase()).filter(Boolean)
        : [];
    const tickers = [...new Set([...heldTickers.map(t => t.toUpperCase()), ...requested])];
