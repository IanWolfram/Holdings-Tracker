...
function loadAgentContext(): string {
  ...
  return ["AGENT.md", "rules.md"]
    .map((f) => {
      try { return fs.readFileSync(path.join(agentDir, f), "utf-8"); } catch { return ""; }
    })
    .filter(Boolean)
    .join("\n\n---\n\n");
}
...
function withOllamaSemaphore<T>(fn: () => Promise<T>): Promise<T> {
...
export class ClassifierService implements IClassifier {
  private agentContext: string | null = null;

  constructor(private readonly cfg: ClassifierConfig) {}

  private getAgentContext(): string {
    if (!this.agentContext) this.agentContext = loadAgentContext();
    return this.agentContext;
  }

  async classify(ticker: string, headline: string, summary: string): Promise<Classification> {
    if (!this.cfg.enabled) return keywordClassify(headline, summary);

    const prompt = `${this.getAgentContext()}
    ...
    try {
      const raw = await withOllamaSemaphore(async () => {
        const res = await fetch(`${this.cfg.baseUrl}/api/generate`, {
          ...
          body: JSON.stringify({ model: this.cfg.model, stream: false, prompt }),
