...
<!-- BEGIN AUTO: Calibration Provenance -->

## Calibration Provenance (auto-generated)

_Generated by `scripts/recalibrate.ts` on 2026-04-28. 66 resolved predictions across 5 tickers. Edit by re-running `npm run recalibrate -- --apply`._

### Catalyst calibration

- **govt-contract** — insufficient data (n=1). Government contract win — 0.85+.
- **earnings-beat** — insufficient data (n=0). Earnings beat + guidance raise — 0.90+.
- **earnings-miss** — insufficient data (n=0). Guidance cut — high-confidence SELL.
- **analyst-upgrade** — insufficient data (n=1). Analyst upgrade w/ price target — 0.75-0.89.
- **analyst-downgrade** — insufficient data (n=0). Analyst downgrade — moderate SELL.
- **regulatory** — insufficient data (n=2). Regulatory — varies by direction.
- **m-and-a** — insufficient data (n=0). M&A — tier-1 BUY signal.
- **macro** — insufficient data (n=1). Macro context — sentiment driven.
- **technical** — n=9, winRate 11.1% vs rule-implied 60.0% (drift -48.9pp) (DOWNWEIGHT). Technical only — sentiment driven. _last updated: 2026-04-28_
- **leadership** — insufficient data (n=2). Executive change — moderate.
- **product-launch** — insufficient data (n=2). Product launch — moderate BUY.
- **partnership** — insufficient data (n=0). Strategic partnership — strong BUY.

### Engine leaderboard

- `backfill-v1` — 7.7% (n=65, avg conf 0.52)

### Confidence-bucket calibration

- 0.5-0.6: actual 3.7% vs implied 55.0% (drift -51.3pp, n=54)
- 0.6-0.7: actual 12.5% vs implied 65.0% (drift -52.5pp, n=8)
- 0.7-0.8: actual 66.7% vs implied 75.0% (drift -8.3pp, n=3)

<!-- END AUTO: Calibration Provenance -->
