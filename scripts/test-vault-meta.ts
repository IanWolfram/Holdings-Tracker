...
const store = new FsVaultStore(vaultPath);
const TICKER = "HOOD";
...
async function main() {
  ...
  console.log("\n=== findTopRecentStories ===");
  const top = await findTopRecentStories(store, TICKER, 8);
  ...
  await store.write(`${TICKER}.preview.md`, content);
  console.log(`\nPreview written to ${TICKER}.preview.md`);
}
